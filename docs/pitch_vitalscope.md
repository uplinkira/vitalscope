...
## 说明
- 当前长期项目名为 `VitalScope`
- `Micro Mirror` 是 `2026-03-14` 深圳黑客松快照阶段使用的原型名
- 下方若出现 `Micro Mirror` 表述，可理解为当前 `VitalScope` 的早期原型指代

## 一句话概括
- 中文：
  - `VitalScope` 是一个用每日健康可视化时间线发现恢复趋势、给出中医与现代医学双参考保健方向，并可通过智能戒指扩展到多模态健康订阅的产品。
- English:
  - `VitalScope` is a health visualization product that tracks recovery trends over time, gives wellness direction with both TCM and modern medical framing, and extends into a smart-ring-powered multimodal subscription service.
...
- `ERC-8004`
  - 对应未来把 `VitalScope` 的分析 agent 或恢复教练 agent 注册成链上身份
...
大家好，我们的项目叫 `VitalScope`。

`VitalScope` 想解决的问题其实很直接：很多人知道自己应该更健康一点，但日常健康管理的反馈太慢了。今天熬夜、压力大、睡眠差，通常不会立刻看到结构化结果；同样，今天开始早睡、补水、调整作息，也很难在短时间内看到可追踪的改善。我们的想法是，把“每天一条健康可视化记录”这件事变成一个可记录、可解释、可形成行动建议的产品入口。

所以我们的基础功能非常简单，也非常容易理解。用户每天打开 `VitalScope`，用摄像头拍一张正脸照片，系统把这张照片和前几天、前几周的记录放到同一条时间线上。我们不把它做成“单次好看不好看”的判断，而是做成“趋势”判断。也就是说，我们更关注的是：你最近是不是在持续变疲惫，还是在慢慢恢复；你的状态变化，是短期波动，还是开始出现值得长期关注的方向。
...
然后，`VitalScope` 不只停在拍照。我们的进阶产品是智能戒指。因为只看照片，能看到的是“外显状态”；但如果我们想真正判断恢复质量和长期健康趋势，还需要照片以外的生理信号。所以在进阶版里，智能戒指会补充像 HRV、静息心率、睡眠恢复这些更底层的指标。这样我们就能把“你看起来累”升级成“你的可视化趋势和恢复系统数据正在一起变差”，或者反过来说，“虽然你自己主观上没感觉，但图像趋势和戒指数据都在往恢复方向走”。这会让建议更稳定，也更适合做长期订阅服务。
...
在主办方 kit 的使用上，我们不是只贴了一个 logo。第一，我们把 `GOAT Testnet3` 的钱包和支付入口直接放进了预购流程，现场可以演示智能戒指如何完成链上支付闭环。第二，我们把 `AgentKit` 里的 `x402` 和 `x402-merchant` 相关能力映射到这个产品里，意味着从支付 intent、签名确认到订单状态、商户管理，这条链路在产品叙事上是完整的。第三，我们把 `ERC-8004` 作为后续健康分析 agent 的身份层来讲，这样 `VitalScope` 未来不只是一个页面，而是一个可以被注册、追踪和积累信誉的健康 agent 服务。为了让 judge 看得更清楚，我们还把本地 `agentkit` 仓库扫描成 `catalog`，直接展示在页面里的 `GOAT / AgentKit Infra Snapshot` 区块。

所以如果用一句话总结，`VitalScope` 想做的是：让健康管理从“偶尔想起来一次”变成“每天打开可视化时间线就能看到趋势，并知道下一步该做什么”。
